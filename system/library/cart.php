...
class Cart {
		...
		if (!$this->data) {
			foreach ($this->session->data['cart'] as $key => $quantity) {
				...
				if ($product_query->num_rows) {
					...
					// считаем цену для букетов

					$price_flowers = 0;

					foreach ($flowers as $id) {
						$flower_query = $this->db->query("SELECT price FROM `" . DB_PREFIX . "buketbase` WHERE id = " . $id . "");
						if(isset($flower_query->row['price'])){
							if(isset($f_quantity[$id])){
								$flower_quantity = $f_quantity[$id][0];
							} else {
								$flower_quantity = 1;
							}

							$price_flowers += $flower_query->row['price'] * $flower_quantity;
						}
					}

					// костыль для наценки букетов без изначальной цены 
					...
					$this->data[$key] = array(
						...
						'stock'           => $stock,
						'price'           => ($price + $option_price + $price_flowers),
						'total'           => ($price + $option_price + $price_flowers) * $quantity,
	...
	public function add($product_id, $qty = 1, $option = array(), $recurring_id = 0, $flowers = array(), $f_quantity = array(), $groups = array()) {
		$this->data = array();
		...
		// состав букета: цветы и их количество

		$product['flowers'] = $flowers;

		$product['f_quantity'] = $f_quantity;
