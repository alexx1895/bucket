...
class ControllerCatalogProduct extends Controller {
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$fquantity = array();
			if (isset($this->request->post['flowers'])) {
				$flowers = $this->request->post['flowers'];
				if(isset($this->request->post['fquantity'])){
					$fquantity = $this->request->post['fquantity'];
				}
			} else {
				$flowers = array();
			}

			$this->model_extension_buket->updateProductFlowers($this->request->get['product_id'], $flowers, $fquantity);
	...
	protected function getList() {
		...
		if (isset($this->request->get['filter_flower'])) {
			$filter_flower = $this->request->get['filter_flower'];
		} else {
			$filter_flower = null;
		}
	...
	protected function getForm() {
		...
		$data['special_entry_price'] = $this->language->get('special_entry_price');
		...
		if (isset($this->request->post['flowers'])) {
			...
			$flowers = $this->model_extension_buket->getProductFlowers($this->request->get['product_id']);
		} else {
			$flowers = array();
		}
		...
		foreach ($flowers as $flower) {
			$flowers_info = $this->model_extension_buket->getFlower($flower['flower_id']);

			if ($flowers_info) {
				$data['flowers'][] = array(
					'id' => $flowers_info['id'],
					'name' => $flowers_info['name'],
		...
		$flowers_colors = $this->model_extension_buket->getProductColors($this->request->get['product_id']);

		foreach ($flowers_colors as $key => $value) {
			$filters[] = $value;
		}
