...
class ControllerModuleCmsblock extends Controller { 
	...
	public function install() { 
	  $this->load->model('cmsblock/info');
       //$this->model_cmsblock_info->createTable(); 
       $this->load->model('setting/setting');
       //$this->model_setting_setting->editSetting('cmsblock', array('cmsblock_status'=>1));
	}
	
	public function uninstall() { 
		$this->load->model('cmsblock/info');
        //$this->model_cmsblock_info->deleteTable();
        $this->load->model('setting/setting');
        //$this->model_setting_setting->editSetting('cmsblock', array('cmsblock_status'=>0));
	}
	
	public function index() {
		$this->load->language('module/cmsblock');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if (!isset($this->request->get['module_id'])) {
				$this->model_extension_module->addModule('cmsblock', $this->request->post);
			} else {
		...
		if (!isset($this->request->get['module_id'])) {
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('module/cmsblock', 'token=' . $this->session->data['token'], 'SSL')
			);
		} else {
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('module/cmsblock', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL')
			);			
		}

		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('module/cmsblock', 'token=' . $this->session->data['token'], 'SSL');
		} else {
			$data['action'] = $this->url->link('module/cmsblock', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
		}
		
		$data['cms_link'] = $this->url->link('module/cmsblock/cmslist', 'token=' . $this->session->data['token'], 'SSL');
		...
		$this->response->setOutput($this->load->view('module/cmsblock/config.tpl', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'module/cmsblock')) {
	...
	public function cmslist() {
		$this->language->load('module/cmsblock');
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('module/cmsblock/cmslist', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			...
			$this->response->redirect($this->url->link('module/cmsblock', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
	...
	protected function getList() {
		...
		$data['insert'] = $this->url->link('module/cmsblock/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$data['delete'] = $this->url->link('module/cmsblock/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');	
		...
		$info_total = $this->model_cmsblock_info->getTotalinfos();
		...
		if($results) {
			foreach ($results as $result) {
				...
				$action[] = array(
					...
					'href' => $this->url->link('module/cmsblock/update', 'token=' . $this->session->data['token'] . '&cmsblock_id=' . $result['cmsblock_id'] . $url, 'SSL')
				);
		...
		$data['sort_title'] = $this->url->link('cmsblock', 'token=' . $this->session->data['token'] . '&sort=id.title' . $url, 'SSL');
		$data['sort_sort_order'] = $this->url->link('cmsblock', 'token=' . $this->session->data['token'] . '&sort=i.sort_order' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('cmsblock', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('module/cmsblock/cmsblock_list.tpl', $data));
	...
	protected function getForm() {
		...
		if (isset($this->request->get['page'])) {
  		...
  		$data['breadcrumbs'] = array();
   		...
   		$data['breadcrumbs'][] = array(
       		'text'      => $this->language->get('heading_title'),
			'href'      => $this->url->link('cmsblock', 'token=' . $this->session->data['token'] . $url, 'SSL'),
      		'separator' => ' :: '
   		);
							
		if (!isset($this->request->get['cmsblock_id'])) {
			$data['action'] = $this->url->link('module/cmsblock/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		} else {
			$data['action'] = $this->url->link('module/cmsblock/update', 'token=' . $this->session->data['token'] . '&cmsblock_id=' . $this->request->get['cmsblock_id'] . $url, 'SSL');
		}
		
		$data['cancel'] = $this->url->link('module/cmsblock', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		// } elseif (isset($this->request->get['cmsblock_id'])) {
			// $data['info_layout'] = $this->model_cmsblock_info->getinfoLayouts($this->request->get['cmsblock_id']);
		// } else {
		...
		$this->response->setOutput($this->load->view('module/cmsblock/cmsblock_form.tpl', $data));
