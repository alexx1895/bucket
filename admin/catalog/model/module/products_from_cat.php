...
class ModelModuleProductsFromCat extends Model {
	...
	public function getProducts($setting) {
		...
		$products_ids = $this->getProductsInCats($setting['categories']);
	...
	private function getProductsInCats( $categories ){

		$implode = implode(",", $categories);

		$query = $this->db->query("SELECT product_id FROM " . DB_PREFIX . "product_to_category WHERE category_id IN( ". $implode ." )");
