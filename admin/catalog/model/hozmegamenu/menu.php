		...
		foreach ($menu_items as $item)
		{
			...
			switch (substr($item, 0, strlen($value[1])))
			{
				...
				case 'LINK': 
					...
					$this->_menuLink .= '<div id ="pt_menu_link" class ="pt_menu"><div class="parentMenu" ><a href="'.($link['link']).'"><span>'.$link['title'].'</span></a></div></div>'.PHP_EOL;	
				case 'CMS': 
...
//        $html[] = '</a>';
	...
	public function drawCustomMenuItem($category, $level = 0, $last = false, $item, $lang_id,$setting) {
		...
		if($category == 169){
			$count = $this->getTotalProducts(array('filter_category_id' => 169));
			$count = ' (' . $count . ')';
			}
			else{
				$count = '';
