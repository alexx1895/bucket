	...
	public function add() {
		...
		if ($product_info) {
			...
			if (isset($this->request->post['f_quantity'])) {
				$f_quantity = $this->request->post['f_quantity'];
			}
			...
			if(isset($this->request->post['iscomp']) && !isset($this->request->post['flowers'])){
				$json['warning'] = "Букет должен содержать цветы";
			...
			if (!$json) {
				$this->cart->add($this->request->post['product_id'], $this->request->post['quantity'], $option, $recurring_id, $flowers, $f_quantity, $groups);

				if(isset($this->request->post['iscomp'])){
					if(!isset($this->session->data['comp_comments'])){
						$this->session->data['comp_comments'] = array();
					}

					$cc = "\nКоментарий к Букету конструктору";

					if(isset($this->request->post['link'])){
						$cc .= "\n ссылка :". $this->request->post['link'];
					}
					if(isset($this->request->post['comment_comp'])){
						$cc .= "\n коментарий клиента :". $this->request->post['comment_comp'];
					} else {
						$cc .= "\n без коментария клиента";
					}
					$cc .="\n\n";
					$this->session->data['comp_comments'][] = $cc;
				}
