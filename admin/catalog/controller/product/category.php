...
class ControllerProductCategory extends Controller {
	public function index() {
		...
		if ($category_info) {
			...
			// if (isset($this->request->get['price_filter'])) {
			// 	$explode = explode("_", $this->request->get['price_filter']);
			// 	$min = $explode[0];
			// 	$max = $explode[1];
			// 	$filter_price = true;
			...
			foreach ($results as $result) {
				...
				if ((float)$result['special'] && $filter_price) {
					if((float)$result['special'] < $min || (float)$result['special'] > $max){
						// filter price
						continue;
					}
				} elseif ($filter_price) {
					if((float)$result['price'] < $min || (float)$result['price'] > $max){
						// filter price
						continue;
					}
				}
