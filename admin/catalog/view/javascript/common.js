jQuery.expr[':'].regex = function(elem, index, match) {
   ...
   return regex.test(jQuery(elem)[attr.method](attr.property));
	...
	function update_price(product_id){
		...
		$('select:regex(name,^(option))').map(function(){
		...
		$('input:regex(name,^(option))').map(function(){
...
$(document).ready(function() {
	...
	$('#submit').click(function(){
		$(this).closest('.subscribe_overlay').fadeOut(200);
		mail = $(this).closest('.subscribe_overlay').find('input').val();
		client.addmail(mail);
	})
	...
	'add': function(product_id) {
		$.ajax({
			...
			success: function(json) {
				console.log(json);
				$('.alert').remove();
				$('#button_compare_comp').fadeIn(200);
				if (json['success']) {
...
var client = {
	'addmail': function(mail) {
		$.ajax({
			url: 'index.php?route=checkout/checkout/addClientMail',
			type: 'post',
			data: 'mail=' + mail,
			dataType: 'json',
			success: function(json) {
