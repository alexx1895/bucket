...
class ModelRetailcrmHistory extends Model
{
    ...
    public function request()
    {
        ...
        //$this->opencartApiClient->clearOrders();
    ...
    {   //print_r($orders);
        foreach ($orders as $order) {
            ...
            $data['customer_status'] = (isset($order['customer']['bad'])&&$order['customer']['bad']!='') ? 1 : 0;
    ...
    protected function createOrders($orders)
    {
        ...
        foreach ($orders as $order) {
            ...
            if (!empty($order['delivery']['address']['region']) && is_int($order['delivery']['address']['region'])) {
                ...
                foreach ($this->zones as $zone) {
                    if (isset($order['delivery']['address']['region']) && $order['delivery']['address']['region'] == $zone['name']) {
                        $region = $zone['zone_id'];
                    }
                }
            }
