...
class ModelCheckoutOrder extends Model {
			...
			if (!$order_info['order_status_id'] && $order_status_id) {
				...
				foreach ($order_product_query->rows as $product) {
					...
					if($order_status_id == 7){
						$this->moveToSpecials($product['product_id']);
					}
	...
	public function moveToSpecials($product_id){
		$this->db->query("INSERT IGNORE INTO `" . DB_PREFIX . "product_to_category` SET category_id = 169, product_id = '". (int)$product_id ."'");

		$query = $this->db->query("SELECT special_price FROM " . DB_PREFIX . "product WHERE product_id = '" . (int)$product_id . "'");

		$price = $query->row['special_price'];

		if($price){
			$this->db->query("DELETE FROM " . DB_PREFIX . "product_special WHERE product_id = '" . (int)$product_id . "'");
			$this->db->query("INSERT INTO " . DB_PREFIX . "product_special SET product_id = '" . (int)$product_id . "', customer_group_id = '1', priority = '0', price = '" . $price . "', date_start = '0000-00-00', date_end = '0000-00-00'");
		}
	}

	public function clear(){
		$this->db->query("DELETE FROM `" . DB_PREFIX . "order` WHERE 1");
		$this->db->query("DELETE FROM `" . DB_PREFIX . "order_product` WHERE 1");
		$this->db->query("DELETE FROM `" . DB_PREFIX . "order_option` WHERE 1");
		$this->db->query("DELETE FROM `" . DB_PREFIX . "order_voucher` WHERE 1");
		$this->db->query("DELETE FROM `" . DB_PREFIX . "order_total` WHERE 1");
		$this->db->query("DELETE FROM `" . DB_PREFIX . "order_history` WHERE 1");
		$this->db->query("DELETE FROM `" . DB_PREFIX . "order_fraud` WHERE 1");
		$this->db->query("DELETE `or`, ort FROM `" . DB_PREFIX . "order_recurring` `or`, `" . DB_PREFIX . "order_recurring_transaction` `ort` WHERE 1 AND ort.order_recurring_id = `or`.order_recurring_id");

		$this->db->query("DELETE FROM `" . DB_PREFIX . "affiliate_transaction` WHERE 1");
