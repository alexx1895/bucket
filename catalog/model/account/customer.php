...
class ModelAccountCustomer extends Model {
	...
	public function addClient($data = array()){
		$query = $this->db->query("SELECT * FROM client WHERE telephone = '" . $this->db->escape($data['telephone']) . "' ");
		if(!empty($query->row)){
			return false;
		}

		$this->db->query("INSERT INTO client SET email = '" . $this->db->escape($data['email']) . "', firstname = '" . $this->db->escape($data['firstname']) . "', telephone = '" . $this->db->escape($data['telephone']) . "'");
	}

	public function addShareDiscount($telephone){
		$query = $this->db->query("SELECT * FROM client WHERE telephone = '" . $this->db->escape($telephone) . "' ");
		if(empty($query->row)){
			return false;
		}

		if($query->row['shared'] == 0 ){
			$this->db->query("UPDATE client SET shared = '1' WHERE telephone = '" . $this->db->escape($telephone) . "' ");
		}
		
	}

	public function getShareDiscount($telephone){
		$query = $this->db->query("SELECT * FROM client WHERE telephone = '" . $this->db->escape($telephone) . "' ");
		if(empty($query->row)){
			return false;
		}

		if($query->row['shared'] == 1 ){
			$this->db->query("UPDATE client SET shared = '2' WHERE telephone = '" . $this->db->escape($telephone) . "' ");
			return true;
		}
		return false;
	}
	public function getShare($telephone){
		$query = $this->db->query("SELECT * FROM client WHERE telephone = '" . $this->db->escape($telephone) . "' ");
		if(empty($query->row)){
			return false;
		}
	...
	public function addClientMail($mail){
		$query = $this->db->query("INSERT INTO oc_mails SET mail = '" . $this->db->escape($mail) . "' ");
	}

	public function editCustomerStatus($telephone, $status = 0){
		$this->db->query("UPDATE client SET status = '" . $status . "' WHERE telephone = '" . $this->db->escape($telephone) . "' ");
	}

	public function checkIsBad($telephone){
		$query = $this->db->query("SELECT status FROM client WHERE telephone = '" . $this->db->escape($telephone) . "' ");
		if(empty($query->row)){
			return false;
		}
