...
class ModelRetailcrmOrder extends Model {
    ...
    {
    	if(!isset($order_data['check']) || !$order_data['check']){
    		return false;
    	}
        ...
        if(!empty($settings['retailcrm_url']) && !empty($settings['retailcrm_apikey'])) {
            ...
            $order['managerComment'] = $order_data['manager'];
            ...
            $order['delivery'] = array(
                ...
                'address' => array(
                    ...
                    'city' => 'Москва',//$order_data['shipping_city'],
            ...
            if(isset($order_data['deliveryTime']) && $order_data['deliveryTime']){
            	$order['delivery']['address']['deliveryTime'] = $order_data['deliveryTime'];
            } else {
            	$order['delivery']['address']['deliveryTime'] = 'уточнить';
            }

            if(isset($order_data['date']) && $order_data['date']){
            	$order['delivery']['date'] = $order_data['date'];
            } 
            ...
            foreach ($orderProducts as $product) {
                ...
                if (isset($this->session->data['share_discount']) && $this->session->data['share_discount']) {
                    $price = $price * 0.8;
                }
